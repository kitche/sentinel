...
impl RequestHandler for WebServer {
    async fn handle_request(
        ...
        let uri_path = path.clone(); // Clone for matching
        ...
        let response = match (&method, uri_path.as_str()) {
            ...
            // Check if proxy is configured and route matches
        ...
        if let Some(ref logger) = self.access_logger {
            ...
            // Fallback to stdout if logging not configured
            println!("{} {} {} {} {}ms", 
                ip, method, path, status, response_time_ms);
        }
